...
// 输出ahci控制器信息
fn ahci_print_info(ahci_dev: &AhciDevice) {
...
// ahci初始化
fn ahci_host_init(ahci_dev: &mut AhciDevice) -> i32 {
    let mut tmp: u32 = 0;
    ...
    for i in 0..ahci_dev.n_ports {
        ...
        if timeout <= 0 {
            log::debug!("spin up cannot finish");
            return -1;
        }
        ...
        if timeout <= 0 {
            log::debug!("port {} sata link timeout", i as u32);
        } else {
            log::debug!("port {} sata link up", i as u32);
        }
...
// ahci填充sgdma
fn ahci_fill_sg(ahci_dev: &AhciDevice, port: u8, buf: *mut u8, mut buf_len: u32) -> u32 {
    let pp: &AhciIoport = &ahci_dev.port[port as usize];
    let mut ahci_sg: *mut AhciSg = pp.cmd_tbl_sg;

    let max_bytes: u32 = AHCI_MAX_BYTES_PER_SG;
    let sg_count: u32 = ((buf_len - 1) / max_bytes) + 1;

    if sg_count > AHCI_MAX_SG {
        log::debug!("too much sg");
        return 0;
    }
...
fn ahci_fill_cmd_slot(pp: &AhciIoport, cmd_slot: u32, opts: u32) {
    let mut cmd_hdr: *mut AhciCmdHdr =
...
fn ahci_exec_ata_cmd(
    ...
    if cmd_slot == 32 {
        log::debug!("cannot find empty command slot");
        return 0;
    }

    if buf_len > AHCI_MAX_BYTES_PER_TRANS {
        log::debug!("max transfer length is {} bytes", AHCI_MAX_BYTES_PER_TRANS);
        return 0;
    }

    unsafe {
        (pp.cmd_tbl as *mut SataFisH2d).write_volatile(*cfis);
    }
    ...
    let opts: u32 = (core::mem::size_of::<SataFisH2d>() as u64 >> 2
        | (sg_count << 16) as u64
...
fn ahci_set_feature(ahci_dev: &AhciDevice, subcmd: u8, action: u8) {
    let port: u8 = ahci_dev.port_idx;
    let cfis: SataFisH2d = SataFisH2d {
...
// 初始化ahci端口
fn ahci_port_start(ahci_dev: &mut AhciDevice, port: u8) -> i32 {
    let pp: &mut AhciIoport = &mut ahci_dev.port[port as usize];
    let port_mmio: u64 = pp.port_mmio;

    let port_status: u32 = ahci_readl(port_mmio + PORT_SCR_STAT);
    if (port_status & 0xf) != 0x3 {
        log::debug!("no link on port {}", port as u32);
        return -1;
    }
    ...
    pp.cmd_tbl_sg = mem as *mut AhciSg;
    ...
    if timeout <= 0 {
        log::debug!("ahci port {} failed to start", port as u32);
        return -1;
    }
    return 0;
}

fn ahci_sata_identify(ahci_dev: &AhciDevice, id: &mut [u16]) {
    let port: u8 = ahci_dev.port_idx;
...
fn ahci_sata_flush_cache(ahci_dev: &AhciDevice) {
    let port: u8 = ahci_dev.port_idx;
    let cfis: SataFisH2d = SataFisH2d {
        fis_type: SATA_FIS_TYPE_REGISTER_H2D,
        pm_port_c: 0x80,
...
fn ahci_sata_flush_cache_ext(ahci_dev: &AhciDevice) {
    let port: u8 = ahci_dev.port_idx;
    let cfis: SataFisH2d = SataFisH2d {
        fis_type: SATA_FIS_TYPE_REGISTER_H2D,
        pm_port_c: 0x80,
...
// 扫描ahci端口
fn ahci_port_scan(ahci_dev: &mut AhciDevice) -> i32 {
    let linkmap: u32 = ahci_dev.port_map_linkup;
    if linkmap == 0 {
        log::debug!("no port device detected");
        return -1;
    }
    ...
    for i in 0..ahci_dev.n_ports {
        if (linkmap >> i & 0x1) != 0 {
            if ahci_port_start(ahci_dev, i) != 0 {
                log::debug!("cannot start port {}", i as u32);
                return -1;
            }
            ahci_dev.port_idx = i;
            break;
...
fn ahci_sata_xfer_mode(ahci_dev: &mut AhciDevice, id: &[u16]) {
    ahci_dev.pio_mask = id[ATA_ID_PIO_MODES as usize] as u32;
    ahci_dev.udma_mask = id[ATA_ID_UDMA_MODES as usize] as u32;
}

fn ahci_sata_init_wcache(ahci_dev: &mut AhciDevice, id: &[u16]) {
