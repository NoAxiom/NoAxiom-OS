...
pub struct CmdReg {
    ...
    ///
    /// 1 - CMD and DATA sent to card through the HOLD Register For more
    /// information, refer to “Host Controller Output Path Timing” on page
    /// 320.
    ...
    /// 	0 - No data transfer expected (read/write) 1 - Data transfer expected
    /// (read/write)
...
pub struct ClockEnableReg {
    ...
    /// to only MMC and SD memory cards; for SDIO cards, if interrupts must
...
pub struct ClockDividerReg {
    ...
    pub clk_divider2: u8,
    pub clk_divider1: u8,
    /// Clock divider-0 value. Clock division is 2* n. For example, value of 0
    /// means
    ///
    /// divide by 2*0 = 0 (no division, bypass), value of 1 means divide by 2*1
    /// = 2, value of “ff” means divide by 2*255 = 510, and so on.
    pub clk_divider0: u8,
}
...
pub struct RawInterruptStatusReg {
    ...
    /// Card[15], and bit[16] is for Card[0]. Writes to these bits clear
    /// them. Value of 1 clears bit and 0 leaves bit intact.
    ///
    /// 0 - No SDIO interrupt from card
    ///
    /// 1 - SDIO interrupt from card
    pub sdiojnterrupt: u16,
    /// Writes to bits clear status bit. Value of 1 clears status bit, and value
    /// of 0 leaves bit intact. Bits are logged regardless of interrupt mask
    /// status.
    pub int_status: u16,
}
...
pub struct BusModeReg {
    ...
    /// beats to be performed in one IDMAC transaction. The IDMAC will
    /// always attempt to burst as specified in PBL each time it starts a
    /// Burst transfer on the host bus. The permissible values are 1,4, 8,
...
pub struct StatusReg {
    ...
    /// Inverted version of raw selected card_data[0] 0 - card data not busy 1 -
    /// card data busy
    pub data_busy: bool,
    /// Raw selected card_data[3]; checks whether card is present 0 - card not
    /// present
    ///
    /// 1 - card present
...
pub struct ControlReg {
    ...
    /// register) 1 - Interrupts are enabled in CE-ATA device (nIEN = 0 in
    /// ATA control register) Software should appropriately write to this
    /// bit after power-on reset or any other reset to CE-ATA device.
    ...
    /// sets bit to reset data state-machine, which is waiting for next
