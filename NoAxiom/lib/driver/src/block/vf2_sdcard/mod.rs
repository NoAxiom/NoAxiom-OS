...
use crate::block::vf2_sdcard::{cmd::Cmd, register::*, utils::*};
...
fn test_read<T: SDIo, S: SleepOps>(io: &mut T) {
    log::debug!("test read, try read 0 block");
    info!("test read");
    ...
    log::debug!("sd header 16bytes: {:x?}", &byte_slice[..2]);
}
...
fn check_version<T: SDIo, S: SleepOps>(io: &mut T) -> u8 {
    ...
    if (resp[0] & 0xaa) == 0 {
        ...
        log::debug!("card version: 1.0");
        return 1;
    }
    log::debug!("card voltage: {:#x?}", resp[0]);
    log::debug!("card version: 2.0");
    2
}
...
fn check_big_support<T: SDIo, S: SleepOps>(io: &mut T) -> bool {
    loop {
        ...
        let ocr = resp[0];
        if ocr.get_bit(31) {
            log::debug!("card is ready");
            if ocr.get_bit(30) {
                log::debug!("card is high capacity");
            } else {
                log::debug!("card is standard capacity");
            }
            break;
        }
        S::sleep_ms(10);
