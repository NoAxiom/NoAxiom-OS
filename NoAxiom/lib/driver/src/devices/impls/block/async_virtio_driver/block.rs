...
//! + inner 成员是 Send 和 Sync 的，_req_type 和 response
//!   成员暂时不会用到，因此从成员变量看来是 Send 和 Sync 的
//! + poll 方法借助了 Mutex 实现内部可变性，在并发场景下多个 poll
//!   操作一起运行的时候，有锁机制保证操作的原子性，因此是 Sync 的
//! 因此个人觉得 BLockFuture 是 Send 和 Sync 的
//!
//! VirtioBlock 设计需求分析：
//! + 需要在并发场景下执行 async_read 或 async_write 或 ack_interrupt 操作，
//! 因此这三个方法都必须是 &self 而不能是 &mut self，因此通过 Mutex
//! 提供内部可变性，并保证并发安全
//! + 需要想清楚哪些操作必须是原子的，必须按顺序来，否则会出问题
//! + 比如多个协程都需要执行
//!   async_read，这时候需要往虚拟队列中添加描述符，然后通知设备，
//! 如果添加描述符和通知设备两个操作不是原子的话，可能会出问题。
//! （这里可能两个操作不应该是原子的，只是举个例子，
//! 说明系统里面可能会有这样的情况）
//!
...
use super::{
    config::*, event::Event, mmio::VirtIOHeader, queue::VirtQueue, util::AsBuf,
    virtio_mm::virtio_phys_to_virt, *,
};
...
impl<const N: usize> VirtIOBlock<N> {
    ...
    pub async fn async_new(header: &'static mut VirtIOHeader) -> Result<VirtIOBlock<N>> {
        ...
        header.begin_init(|f| {
            ...
            log::debug!("[virtio] block device features: {:?}", features);
            // 对这些 features 进行谈判
        ...
        log::debug!("[virtio] config: {:?}", config);
        log::debug!(
