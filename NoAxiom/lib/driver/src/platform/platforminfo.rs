...
fn walk_dt(fdt: Fdt) -> PlatformInfo {
    ...
    let model = Arch::model().unwrap_or(fdt.root().model().to_string());
...
fn walk(fdt: Fdt, machine: &mut PlatformInfo) {
    for node in fdt.all_nodes() {
        ...
        if node.name.starts_with(&Arch::memory_name()) {
            log::trace!("[fdt]: memory node {}", node.name);
            ...
            log::trace!("[fdt]: plic node {}", node.name);
            let reg = node.reg().unwrap();
            reg.for_each(|x| {
                machine.plic = Range {
                    start: x.starting_address as usize,
            ...
            log::trace!("[fdt]: clint node {}", node.name);
            let reg = node.reg().unwrap();
            reg.for_each(|x| {
                machine.clint = Range {
                    start: x.starting_address as usize,
            ...
            log::trace!("[fdt]: chose node {}", node.name);
