...
use core::{marker::PhantomPinned, mem::forget, pin::Pin, task::Poll};
...
pub(super) struct RawRwLock {
    ...
    ///
    /// The least significant bit (`WRITER_BIT`) is set to 1 when a writer is
    /// holding the lock or trying to acquire it.
    ///
    /// The upper bits contain the number of currently active readers. Each
    /// active reader increments the state by `ONE_READER`.
    state: AtomicUsize,
}
...
impl RawRwLock {
    ...
    pub(super) fn try_read(&self) -> bool {
        ...
        loop {
            // If there's a writer holding the lock or attempting to acquire it, we cannot
            // acquire a read lock here.
    ...
    /// Returns `true` iff a the upgradable read lock was successfully upgraded
    /// to a write lock.
    ///
    /// # Safety
    ///
        ...
        loop {
            match this.state.as_mut().project() {
                WriteStateProj::Acquiring { lock } => {
                    ...
                    // Set `WRITER_BIT` and create a guard that unsets it in case this future is
                    // canceled.
