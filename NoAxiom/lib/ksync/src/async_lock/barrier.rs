...
impl Barrier {
    ...
    ///
    /// Barriers are reusable after all tasks have synchronized, and can be used
    /// continuously.
    ///
    /// Returns a [`BarrierWaitResult`] indicating whether this task is the
    /// "leader", meaning the last task to call this method.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// [`wait`][`Barrier::wait`] method, this method will block the current
    /// thread until the wait is complete.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a barrier can be used in both
