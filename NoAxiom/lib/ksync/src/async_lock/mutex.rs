...
use crate::async_lock::abort;

/// An async mutex.
///
/// The locking mechanism uses eventual fairness to ensure locking will be fair
/// on average without sacrificing performance. This is done by forcing a fair
/// lock whenever a lock operation is starved for longer than 0.5 milliseconds.
///
/// # Examples
///
/// ```
...
impl<T: ?Sized> Mutex<T> {
    ...
    /// # Blocking
    ///
    /// Rather than using asynchronous waiting, like the [`lock`][Mutex::lock]
    /// method, this method will block the current thread until the lock is
    /// acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a mutex can be used in both
    ...
    ///
    /// If the mutex could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, a guard is returned that releases the mutex
    /// when dropped.
    ///
    /// # Examples
    ///
    ...
    ///
    /// Since this call borrows the mutex mutably, no actual locking takes place
    /// -- the mutable borrow statically guarantees the mutex is not already
    /// acquired.
    ///
    /// # Examples
    ///
    ...
    /// # Safety
    ///
    /// This function is intended to be used only in the case where the mutex is
    /// locked, and the guard is subsequently forgotten. Calling this while
    /// you don't hold a lock on the mutex will likely lead to UB.
...
impl<T: ?Sized> Mutex<T> {
    ...
    /// Acquires the mutex and clones a reference to it using the blocking
    /// strategy.
    ///
    /// Returns an owned guard that releases the mutex when dropped.
    ///
    ...
    /// [`lock_arc`][Mutex::lock_arc] method, this method will block the
    /// current thread until the lock is acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a mutex can be used in both
    /// asynchronous and synchronous contexts. Calling this method in an
    ...
    ///
    /// If the mutex could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, an owned guard is returned that releases the
    /// mutex when dropped.
    ///
    /// # Examples
    ///
...
/// `pin_project_lite` doesn't support `#[cfg]` yet, so we have to do this
/// manually.
struct Start {
    #[cfg(all(feature = "std", not(target_family = "wasm")))]
