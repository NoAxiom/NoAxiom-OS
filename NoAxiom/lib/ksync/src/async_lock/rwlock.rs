...
use self::{
    ...
    raw::{RawRwLock, RawUpgrade},
};

/// An async reader-writer lock.
///
/// This type of lock allows multiple readers or one writer at any point in
/// time.
///
/// The locking strategy is write-preferring, which means writers are never
/// starved. Releasing a write lock wakes the next blocked reader and the next
/// blocked writer.
///
/// # Examples
///
...
impl<T> RwLock<T> {
    ...
    ///
    /// If a read lock could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, a guard is returned that releases the lock when
    /// dropped.
    ///
    /// # Examples
    ///
    ...
    /// # Blocking
    ///
    /// Rather than using asynchronous waiting, like the
    /// [`read_arc`][`RwLock::read_arc`] method, this method will block the
    /// current thread until the read lock is acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a lock can be used in both
    /// asynchronous and synchronous contexts. Calling this method in an
    /// asynchronous context may result in a deadlock.
    ///
    /// # Examples
    ///
    /// ```
...
impl<T: ?Sized> RwLock<T> {
    ...
    ///
    /// Rather than using asynchronous waiting, like the
    /// [`read`][`RwLock::read`] method, this method will block the current
    /// thread until the read lock is acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a lock can be used in both
    /// asynchronous and synchronous contexts. Calling this method in an
    ...
    /// Attempts to acquire a read lock with the possiblity to upgrade to a
    /// write lock.
    ///
    /// If a read lock could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, a guard is returned that releases the lock when
    /// dropped.
    ///
    /// Upgradable read lock reserves the right to be upgraded to a write lock,
    /// which means there can be at most one upgradable read lock at a time.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// which means there can be at most one upgradable read lock at a time.
    ///
    /// Note that attempts to acquire an upgradable read lock will block if
    /// there are concurrent attempts to acquire another upgradable read
    ...
    ///
    /// Rather than using asynchronous waiting, like the
    /// [`upgradable_read`][`RwLock::upgradable_read`] method, this method
    /// will block the current thread until the read lock is acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a lock can be used in both
    /// asynchronous and synchronous contexts. Calling this method in an
    ...
    ///
    /// Rather than using asynchronous waiting, like the
    /// [`upgradable_read_arc`][`RwLock::upgradable_read_arc`] method, this
    /// method will block the current thread until the read lock is acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a lock can be used in both
    /// asynchronous and synchronous contexts. Calling this method in an
    ...
    /// Attempts to acquire an owned, reference-counted read lock with the
    /// possiblity to upgrade to a write lock.
    ///
    /// If a read lock could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, a guard is returned that releases the lock when
    /// dropped.
    ...
    ///
    /// If a write lock could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, a guard is returned that releases the lock when
    /// dropped.
    ///
    /// # Examples
    ...
    /// # Blocking
    ///
    /// Rather than using asynchronous waiting, like the [`write`] method, this
    /// method will block the current thread until the write lock is
    /// acquired.
    ///
    /// This method should not be used in an asynchronous context. It is
    /// intended to be used in a way that a lock can be used in both
    ...
    ///
    /// Rather than using asynchronous waiting, like the
    /// [`write_arc`][RwLock::write_arc] method, this method will
    ...
    ///
    /// Since this call borrows the lock mutably, no actual locking takes place.
...
impl<'a, T: ?Sized> RwLockUpgradableReadGuard<'a, T> {
    ...
    ///
    /// If a write lock could not be acquired at this time, then [`None`] is
    /// returned. Otherwise, an upgraded guard is returned that releases the
    /// write lock when dropped.
    ///
    ...
    /// # Blocking
    ///
    /// This function will block the current thread until it is able to acquire
    /// the write lock.
    ///
    /// # Examples
    ///
...
/// An owned, reference-counting guard that releases the upgradable read lock
/// when dropped.
#[clippy::has_significant_drop]
