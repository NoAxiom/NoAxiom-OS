...
impl RngCore for Rng {
    fn next_u32(&mut self) -> u32 {
        let next = self.seed + get_time_duration().as_micros() as usize;
        self.seed = next;
        next as u32
    }
...
impl Rng {
    pub fn positive_u32(&mut self) -> u32 {
        let mut next = self.seed + get_time_duration().as_micros() as usize;
        while (next & 0xff) as u32 == 0 {
            self.seed = next;
            next = self.seed + get_time_duration().as_micros() as usize;
        }
        (next & 0xff) as u32
    }
    pub fn positive_usize(&mut self) -> usize {
        let mut next = self.seed + get_time_duration().as_micros() as usize;
        while next & 0xff == 0 {
            self.seed = next;
            next = self.seed + get_time_duration().as_micros() as usize;
