...
use virtio_mm::{virtio_dma_alloc, virtio_dma_dealloc, virtio_phys_to_virt};
...
impl Future for DMAAllocFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let paddr = virtio_dma_alloc(self.pages as usize);
        match paddr {
            0 => {
                self.count += 1;
...
impl DMA {
    ...
    pub fn start_virtual_address(&self) -> usize {
        virtio_phys_to_virt(self.paddr as usize)
    }
...
impl Drop for DMA {
    fn drop(&mut self) {
        let err = virtio_dma_dealloc(self.paddr, self.pages);
