...
///
/// 如果一个监听者在没有收到任何消息的情况下走到生命周期的尽头，`dropping`
/// 会通知 其他监听了同一个 [`Event`] 的监听者。
///
/// 监听者们的注册和被通知操作遵循`先进先出`规律，保证公平性。
pub struct Event {
    /// A pointer to heap-allocated inner state.
    ///
    /// This pointer is initially null and gets lazily initialized on first use.
    /// Semantically, it is an `Arc<Inner>` so it's important to keep in
    /// mind that it contributes to the [`Arc`]'s reference count.
...
impl Event {
    ...
    pub fn notify_relaxed(&self, n: usize) {
        if let Some(inner) = self.try_inner() {
            // Notify if there is at least one unnotified listener and the number of
            // notified listeners is less than `n`.
            if inner.notified.load(Ordering::Acquire) < n {
                inner.lock().notify(n);
            }
        }
    ...
    /// 数量可以是零或者超过监听者的数量
    ///
    /// Unlike [`Event::notify_additional()`], this method does not emit a
    /// `SeqCst` fence.
    ///
    /// # Examples
    ///
