...
impl VirtIOBlk<'_> {
    ...
    /// Read a block in a non-blocking way which means that it returns
    /// immediately.
    ///
    /// # Arguments
    ///
    ...
    ///   which contains the status of the requests. The caller can safely read
    ///   the variable only after the request is ready.
    ///
    /// # Usage
    ///
    ...
    ///
    /// After the request is ready, `resp` will be updated and the caller can
    /// get the status of the request(e.g. succeed or failed) through it.
    /// However, the caller **must not** spin on `resp` to wait for it to
    /// change. A safe way is to read it after the same token as this method
    /// returns is fetched through [VirtIOBlk::pop_used()], which means that
    /// the request has been ready.
    ///
    /// # Safety
    ///
    /// `buf` is still borrowed by the underlying virtio block device even if
    /// this method returns. Thus, it is the caller's responsibility to
    /// guarantee that `buf` is not accessed before the request is completed
    ...
    //// Write a block in a non-blocking way which means that it returns
    //// immediately.
    /// # Arguments
    ///
    /// * `block_id` - The identifier of the block to write.
    /// * `buf` - The buffer in the memory containing the data to write to the
    ///   block.
    /// * `resp` - A mutable reference to a variable provided by the caller
    ///   which contains the status of the requests. The caller can safely read
    ///   the variable only after the request is ready.
    ...
    /// During an interrupt, it fetches a token of a completed request from the
    /// used ring and return it. If all completed requests have already been
    /// fetched, return Err(Error::NotReady).
    pub fn pop_used(&mut self) -> Result<u16> {
        self.queue.pop_used().map(|p| p.0)
    }

    /// Return size of its VirtQueue.
    /// It can be used to tell the caller how many channels he should monitor
    /// on.
